...
OriginalPrice = 899.99, SalePrice = 849.99, IsFreeShipping = true, ImageUrl = "https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/mbp14-spacegrayselect-202301?wid=2000&hei=2000&fmt=jpeg&qlt=90" },
...
= "Samsung", OriginalPrice = 199.99, SalePrice = 179.99, IsFreeShipping = true, ImageUrl = "https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/mbp14-spacegray-select-202301?wid=2000&hei=2000&fmt=jpeg&qlt=90" },
...
public IActionResult Index()
        ...
        [HttpGet]
        public IActionResult Filters(string searchValue, string brands, string category, double? minPrice, double? maxPrice, bool? isFreeShipping, int page = 1)
        {
            int pageSize = 8;
            var result = products.AsQueryable();

            if (!string.IsNullOrEmpty(searchValue))
                result = result.Where(p => p.Name != null && p.Name.ToLower().Contains(searchValue.ToLower()));

            if (!string.IsNullOrEmpty(brands))
            {
                var brandList = brands.Split(",");
                result = result.Where(p => p.Brand != null && brandList.Contains(p.Brand));
            }
            ...
            if (isFreeShipping == true)
                result = result.Where(p => p.IsFreeShipping);
